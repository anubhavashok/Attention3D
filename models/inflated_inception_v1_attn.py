...
class Inception3D(nn.Module):
    def __init__(self, num_classes=157):
        ...
        self.conv4 = nn.Conv3d(1024, num_classes, kernel_size=(1, 1, 1), stride=(1, 1, 1))#BasicConv3d(1024, num_classes, (1, 1, 1))
    ...
    def forward(self, x, attn=None):
        ...
        outputs.append(x)
        #print(x.size())
        #print('Applying attn: ', x.size(), attn[4].size())
        x = x.mul(attn[4])
        x = self.conv3(x)
        outputs.append(x)
        #print(x.size())
        ...
        #print(x.size())
        x = self.inc2(x)
        #print('Applying attn: ', x.size(), attn[3].size())
        x = x.mul(attn[3])
        outputs.append(x)
        #print(x.size())
        x = self.maxpool3(x)
        ...
        #print(x.size())
        x = self.inc5(x)
        #print('Applying attn: ', x.size(), attn[2].size())
        x = x.mul(attn[2])
        outputs.append(x)
        #print(x.size())
        x = self.inc6(x)
        ...
        x = self.maxpool4(x)
        x = self.inc8(x)
        #print('Applying attn: ', x.size(), attn[1].size())
        x = x.mul(attn[1])
        outputs.append(x)
        #print(x.size())
        x = self.inc9(x)
        #print('Applying attn: ', x.size(), attn[0].size())
        x = x.mul(attn[0])
        outputs.append(x)
        x = self.padding(x)
        x = self.avgpool(x)
